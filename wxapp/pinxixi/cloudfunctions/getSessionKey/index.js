...
const getSessionKey = (code) => {
  ...
  const opts = {
    appid: 'wx08a3692b6aec129e',
    secret: '4ba25802fe6acf6ecb7bfad284b01fb9',
    js_code: code,
    grant_type: "authorization_code"
  }
...
exports.main = async (event, context) => {
  ...
  try {
    ...
    // 存入用户信息
    const findByOpenid = await db.collection('people')
      .where({
        openid
      })
      .get()
    if (!findByOpenid.data.length) {
      const insertRes = await db.collection('people')
        .add({
          data: {
            openid,
            avatarUrl
